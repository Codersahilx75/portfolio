...
function Skills() {
  return (
   <section  id='skills'>
     <div className="text-center py-12 bg-white dark:bg-gray-900 transition-colors duration-300">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-6 max-w-7xl mx-auto">
        {skills.map((skill, index) => (
          <div key={index} className="flex flex-col items-center">
            ...
            <p className="mt-2 text-sm sm:text-base text-gray-700 dark:text-gray-300">{skill.name}</p>
          </div>
        ))}
      </div>
    </div>
